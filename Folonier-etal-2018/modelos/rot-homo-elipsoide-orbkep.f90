      ...
      open (1,file='datos2.dat')
      open (2,file='disipa2.dat')
      open (3,file='datos-atractor2.dat')
...
!     lazo principal de integracion y precision de la integracion.
      time0  = 0.0d0
      tfinal = 3.0d3      ! final integration time (initial is = 0)
      htime  = per/1.0d2  ! output interval
      h      = per/1.0d2  ! initial timestep
      ll     = 13

!     preparacion para la salida de datos
      nout   = 1.0d5         ! numero de pasos
      numpas = tfinal/htime  ! numero inicial de pasos
      if (numpas.lt.nout) nout = numpas
      flagout = numpas/nout
      flag    = 1.0d0
...
!
      call energia (y,w)
      call atractor (tau,znu,zdel,zer,zez)
      write(1,110) tau,y(1)/rad,y(2)/rad,y(3),y(4)
      write(3,110) tau,znu/rad,zdel/rad,zer,zez
      write(2,110) tau,w
      write(*,100) tau,y(1)/rad,y(2)/rad,y(3),y(4)
...
!
      ...
      f(1) =f(1)-uno/3.75d0*mtot*rtot**2*omega*gama*(epola-epz)
...
!
      ...
      si = dsin(dos*del)
!
      E0      = 0.08d0*g*mtot**2/gama/rtot
      dotErho = gama*(ejean*co-ep)
      dotang  = gama*ejean*si/ep
      dotEz   = gama*(epola-epz)
      Etot  =-E0*(dotErho**2+ep**2*dotang**2+cuatro/tres*dotEz**2)
      Erot1 =-gama*E0*dos*omega*ejean*ep*si
      Erot2 =-uno/7.5d0*mtot*rtot**2*omega**2*dotEz
      Erot  = Erot1+Erot2
      Eorb  = Etot-Erot
...
!
      ...
      w(4) = dadt*std/sty*uam/1.0d3  ! da/dt  [km/yr]
      w(5) = dedt*std/sty            ! de/dt  [1/yr]
